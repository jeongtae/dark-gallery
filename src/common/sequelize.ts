...
export type ItemAttributes = {
   ...
   * DB에는 경로가 POSIX 방식인 슬래시로 구분되어 저장되며, 읽거나 쓸 때 자동으로 현재 플랫폼에 맞게 변환되지만,
   * `raw: true`옵션으로 `SELECT`할 시에는 변환되지 않으므로 직접 변환해야한다.
   * `validate: false`옵션으로 `INSERT` 또는 `UPDATE`하는 경우에도 변환되지 않는다.
   * @example
   * "."
   * "foo"
   * "foo/bar"
